# A3_erik_SIMPLIFIED.py
# Simplified version addressing critical issues:
# 1. Simple race-based fitness (finish bonus + speed)
# 2. Standard EA parameters (no over-engineering)
# 3. Proper controller EA (no adaptive search)
...
# =========================
SCRIPT_NAME = "A3_erik_simplified"
CWD = Path(".").resolve()
...
# EA parameters - STANDARD VALUES
BODY_POP_SIZE = 20            # Standard population
BODY_N_GEN = 25               # Reasonable number of generations
BODY_TOURNSIZE = 3            # Standard tournament
BODY_CXPB = 0.7               # Standard crossover
BODY_MUTPB = 0.3              # Standard mutation
BODY_SBX_ETA = 15.0           # Standard SBX
BODY_ELITE_K = 3              # Keep top 3 elites

# Controller EA - FIXED BUDGET
CTRL_POP_SIZE = 12            # Fixed population
CTRL_N_GEN = 8                # Fixed generations
CTRL_TOURNSIZE = 3
CTRL_CXPB = 0.8
CTRL_MUTPB = 0.25
CTRL_SBX_ETA = 15.0
...
VERBOSE = False
_CTRL_UNLIM_SCALE = np.pi / 2  # FIXED: was using pi instead of pi/2
...
# =========================
# SIMPLE RACE FITNESS (ISSUE #1 - FIXED)
# =========================
...
def compute_race_fitness(x_hist: list[float], t_hist: list[float], track_length: float) -> tuple[float, dict]:
    """
    SIMPLE RACE FITNESS (Assignment requirement):
    1. If finished: HUGE bonus + speed reward (lower time = better)
    2. If not finished: reward distance traveled

    NO complex multi-component fitness, NO quality metrics, NO milestones
    Just: Did it finish? How fast? How far?
    """
    if len(x_hist) < 2:
    ...
    if finished:
        # Calculate actual time to finish
        ttf = _estimate_ttf(x_hist, t_hist, track_length)

        # HUGE finish bonus + speed reward
        # 1,000,000 = finish bonus (dominates)
        # + track_length/ttf = speed reward (faster = higher score)
        ...
        # Didn't finish: fitness = distance traveled
        # Small speed bonus to break ties
...
# =========================
# STANDARD CONTROLLER EA (ISSUE #3 - FIXED)
# =========================
...
def evolve_controller_for_body(body_geno, verbose=False):
    """
    STANDARD CONTROLLER EA (no adaptive budget, no random search)
    Just a clean EA with fixed population and generations.
    """
...
def probe_best_metrics(arch: BodyArchitecture, theta: np.ndarray) -> tuple[float, float]:
    ...
    # Extract distance from fitness
    # Since we use simple fitness, we need to re-run to get detailed metrics
    # For now, estimate distance from fitness
    if fit > 1_000_000:
        return TRACK_LENGTH, 0.0  # Finished
    else:
        dist = fit / 10.0  # Rough estimate
...
# MAIN EA LOOP
# =========================
def run_simplified_ea():
    """
    Simplified EA with:
    - Standard parameters (no over-engineering)
    - Simple race fitness
    - Standard controller EA
    """
    random.seed(SEED); np.random.seed(SEED)
    console.log(f"[SIMPLIFIED EA] Starting co-evolution")
    console.log(f"[Config] Body: pop={BODY_POP_SIZE}, gen={BODY_N_GEN}")
    console.log(f"[Config] Controller: pop={CTRL_POP_SIZE}, gen={CTRL_N_GEN}")
...
def main():
    console.log("="*70)
    console.log("[SIMPLIFIED VERSION - Fixes Issues #1, #2, #3]")
    console.log("="*70)
    console.log("Changes:")
    console.log("  1. Simple race fitness (finish bonus + speed OR distance)")
    console.log("  2. Standard EA parameters (pop=20, gen=25)")
    console.log("  3. Standard controller EA (no adaptive search)")
    console.log("  4. Fixed actuator scaling (pi/2 instead of pi)")
    console.log("="*70 + "\n")

    best, fit_curve, dist_curve = run_simplified_ea()
