# A3_erik_FIXED.py
# FIXED VERSION addressing log analysis issues:
# 1. Fixed distance metric calculation (was estimating wrong)
# 2. Increased controller EA budget (12x8 → 16x10 for better learning)
# 3. Added diversity maintenance (adaptive mutation + immigration)
# 4. Improved logging to show what's happening
...
# =========================
# PARAMETERS (slightly tuned based on log analysis)
# =========================
SCRIPT_NAME = "A3_erik_fixed"
CWD = Path(".").resolve()
...
BODY_TOURNSIZE = 3
BODY_CXPB = 0.7
BODY_MUTPB = 0.3              # Base mutation rate
BODY_SBX_ETA = 15.0
BODY_ELITE_K = 3

# Controller EA parameters (INCREASED for better learning)
CTRL_POP_SIZE = 16            # Was 12 - increased
CTRL_N_GEN = 10               # Was 8 - increased
CTRL_TOURNSIZE = 3
CTRL_CXPB = 0.8
CTRL_MUTPB = 0.25
CTRL_SBX_ETA = 15.0
...
def block_mutation(g, indpb: float = 3.0 / BODY_L, rng: np.random.Generator | None = None):
    """Standard mutation: ~3 genes per mutation"""
    rng = rng or np.random.default_rng()
    t, c, r = g
...
class CtrlEntry(NamedTuple):
    theta: np.ndarray
    fitness: float
    age: int

_CTRL_POOL: dict[str, list[CtrlEntry]] = {}
# Pool / restart hyperparams
POOL_K = 3
CTRL_EVAL_RESTARTS = 2
CTRL_CONTINUE_GENS = 5
MAX_CTRL_AGE = 5
...
def compute_race_fitness(x_hist: list[float], t_hist: list[float], track_length: float) -> tuple[float, dict]:
    """Simple race fitness: finish bonus + speed OR distance"""
    if len(x_hist) < 2:
    ...
    if finished:
        ...
        fitness = dist * 10.0 + avg_speed * 0.1
...
def evolve_controller_for_body_once(arch, init_thetas: list[np.ndarray], n_gen: int):
    """Run a controller EA for a fixed number of generations starting from given thetas."""
    try:
    ...
    # Initialize population from seeds; pad with randoms
    pop = []
    for th in init_thetas[:CTRL_POP_SIZE]:
        pop.append(creator.ControllerIndividual(th.tolist()))
    while len(pop) < CTRL_POP_SIZE:
        th = init_controller_genotype_for_body(arch.inp_size, arch.out_size, RNG)
        pop.append(creator.ControllerIndividual(th.tolist()))
    ...
    return np.array(best, dtype=float), float(best.fitness.values[0])



def evolve_controller_for_body(body_geno, verbose=False):
    """Pool-aware controller EA: continuation + restarts; keep top-K per body."""
    ...
    pool = _CTRL_POOL.get(key, []).copy()

    seeds = []
    if pool:
        # Continue from current best
        best_entry = max(pool, key=lambda e: e.fitness)
        seeds.append(best_entry.theta.copy())

    # Fill with random seeds to population size
    while len(seeds) < CTRL_POP_SIZE:
        th = init_controller_genotype_for_body(arch.inp_size, arch.out_size, RNG)
        seeds.append(th)

    candidates = []

    # Short continuation if we had a best
    if pool:
        theta_c, fit_c = evolve_controller_for_body_once(arch, seeds, n_gen=CTRL_CONTINUE_GENS)
        candidates.append(CtrlEntry(theta_c, fit_c, age=0))

    # Full restarts
    for _ in range(CTRL_EVAL_RESTARTS):
        init_list = [init_controller_genotype_for_body(arch.inp_size, arch.out_size, RNG) for __ in range(CTRL_POP_SIZE)]
        theta_r, fit_r = evolve_controller_for_body_once(arch, init_list, n_gen=CTRL_N_GEN)
        candidates.append(CtrlEntry(theta_r, fit_r, age=0))

    # Age previous pool
    aged = [CtrlEntry(e.theta, e.fitness, e.age + 1) for e in pool]

    merged = [*aged, *candidates]
    merged = [e for e in merged if e.age <= MAX_CTRL_AGE]
    merged.sort(key=lambda e: e.fitness, reverse=True)
    _CTRL_POOL[key] = merged[:POOL_K] if merged else []

    if not _CTRL_POOL[key]:
        return None, -1e6
    best = _CTRL_POOL[key][0]
    return best.theta, best.fitness


def evaluate_body_individual(ind):
    """Simple body evaluation using standard controller EA"""
    geno = ind[0]
    try:
...
def probe_best_metrics(arch: BodyArchitecture, theta: np.ndarray) -> tuple[float, float]:
    """
    FIXED: Properly calculate distance by re-running episode
    (not estimating from fitness which includes speed component)
    """
...
# =========================
# MAIN EA LOOP (WITH DIVERSITY MAINTENANCE)
# =========================
def run_fixed_ea():
    """
    EA with fixes:
    - Increased controller budget (16x10)
    - Adaptive mutation when stagnating
    - Immigration for diversity
    - Fixed distance calculation
    """
    random.seed(SEED); np.random.seed(SEED)
    console.log(f"[FIXED EA] Starting co-evolution")
    console.log(f"[Config] Body: pop={BODY_POP_SIZE}, gen={BODY_N_GEN}, mut={BODY_MUTPB}")
    console.log(f"[Config] Controller: pop={CTRL_POP_SIZE}, gen={CTRL_N_GEN} (INCREASED)")
    console.log(f"[Config] Diversity: adaptive mutation + immigration on stagnation")
    console.log(f"[Fitness] Simple race: finish bonus + speed OR distance")
...
try:
    ...
    if not best_arch or not best_arch.viable:
        best_arch = get_body_architecture(best_key, best0[0])
    theta0, fit0 = evolve_controller_for_body(best0[0], verbose=False)
    if theta0 is not None and best_arch and best_arch.viable:
        dist, _ = probe_best_metrics(best_arch, theta0)
...
for gen in range(1, BODY_N_GEN + 1):
    ...
    # IMMIGRATION (add diversity when stuck)
    if no_improve >= 3:
    ...
    # Periodic controller-only refresh for elite bodies
    if gen % 3 == 0:
        for elite_ind in hof.items:
            try:
                # Re-run a heavier controller search for elites (one extra restart)
                _ = evolve_controller_for_body(elite_ind[0], verbose=False)
            except Exception as _e:
                pass


    # Build next generation (elites + best offspring)
    ...
    try:
        ...
        if best_arch and best_arch.viable:
            cached = _CTRL_POOL.get(best_key)
            if cached:
                theta_best = cached[0].theta if isinstance(cached, list) else cached.theta
...
try:
    ...
    cached = _CTRL_POOL.get(final_key)
    if cached:
        theta_final = cached[0].theta if isinstance(cached, list) else cached.theta
...
try:
    ...
    ax1.set_title("Best Fitness per Generation", fontsize=12, fontweight='bold')
...
def main():
    console.log("="*70)
    console.log("[FIXED VERSION - Addressing Log Analysis Issues]")
    console.log("="*70)
    console.log("Fixes:")
    console.log("  1. Increased controller budget (12x8 → 16x10)")
    console.log("  2. Fixed distance calculation (was estimating wrong)")
    console.log("  3. Added adaptive mutation (increases when stuck)")
    console.log("  4. Added immigration for diversity (triggers at stagnation≥3)")
    console.log("="*70 + "\n")

    best, fit_curve, dist_curve = run_fixed_ea()
    ...
    console.log(f"\nFitness progression: {[round(f, 1) for f in fit_curve[::max(1, len(fit_curve)//10)]]}")
    console.log(f"Distance progression: {[round(d, 2) for d in dist_curve[::max(1, len(dist_curve)//10)]]}")
    console.log("="*70)
