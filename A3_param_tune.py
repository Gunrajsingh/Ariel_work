...
BODY_POP_SIZE = 5 # 30           # Smaller: focus on quality over quantity
BODY_N_GEN = 3 # 30              # More generations
BODY_TOURNSIZE = 3
BODY_CXPB = 0.7
BODY_MUTPB = 0.5
BODY_SBX_ETA = 15.0
...
CTRL_POP_SIZE = 5 # 30           # DOUBLED from aggressive
...
# Budget
TUNE_N_CANDIDATES = 8       # total random configs to try (includes baseline)
TUNE_TOP_K = 2              # keep top-K for stage 2
TUNE_GEN_STAGE1 = 1         # generations for stage 1
TUNE_GEN_STAGE2 = 3         # generations for stage 2
TUNE_POP = max(5, min(8, BODY_POP_SIZE))  # small pop for tuning proxy
...
def _score_config(cfg, type, gen_budget, seed=42):
    """
...
def tune_hparams(type, param_grid):
    ...
    for i, cfg in enumerate(candidates):
        s = _score_config(cfg, type, gen_budget=TUNE_GEN_STAGE1, seed=100+i)
        scores.append((s, cfg))
    ...
    finals = []
    for rank, (s, cfg) in enumerate(top, 1):
        s2 = _score_config(cfg, type, gen_budget=TUNE_GEN_STAGE2, seed=777+rank)
...
def main():
    ...
    body_grid = {
        "BODY_CXPB": [0.6, 0.7, 0.8],
        "BODY_MUTPB": [0.15, 0.2, 0.25, 0.3],
